...
async function main() {
  const token = process.env.GITHUB_TOKEN;
  console.log(token ? '✅ GITHUB_TOKEN is present in environment.' : '❌ GITHUB_TOKEN is MISSING in environment.');
  
  if (!token) {
    console.error('❌ GITHUB_TOKEN is not set. Please check your repository secrets.');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`✅ Success! repos.json updated with ${repos.length} repositories.`);
  } catch (err) {
