...
const ExperienceItem = ({ data }: { data: any }) => {
  return (
    <div className="relative pl-12 md:pl-20 pb-24 last:pb-0 group">
      ...
      <div className="absolute left-0 md:left-1 top-2.5 w-4 h-4 rounded-full bg-[#020c1b] border-2 border-cyan-500/50 group-hover:border-cyan-400 transition-colors duration-500 z-10 shadow-[0_0_10px_rgba(34,211,238,0)] group-hover:shadow-[0_0_15px_rgba(34,211,238,0.4)]">
      ...
      <div className="flex flex-col gap-2 transition-transform duration-500 group-hover:translate-x-2">
        ...
        {/* Timestamp */}
        <span className="text-sm font-bold tracking-[0.2em] text-cyan-400/80 mb-2 uppercase">{data.period}</span>
        
        {/* Content without the heavy card */}
        <div className="space-y-6">
          <div>
            <h3 className="text-3xl sm:text-4xl font-black text-white tracking-tight mb-2 group-hover:text-cyan-50 transition-colors">{data.role}</h3>
            <p className="text-xl font-medium text-cyan-100/50">{data.company}</p>
          </div>
          
          {/* Points */}
          <ul className="space-y-4 max-w-3xl">
            {data.points.map((point: string, i: number) => (
              <li key={i} className="flex items-start gap-4 text-cyan-100/70 text-lg leading-relaxed group/item hover:text-white transition-colors">
...
export default function Experience() {
  return (
    <div className="screen-section relative bg-[#020c1b] text-white pt-16 pb-32 overflow-hidden">
      <div id="experience" className="absolute top-0" />
      
      {/* Background Parallax: The Chase */}
      <div className="absolute inset-0 pointer-events-none opacity-20 overflow-hidden flex items-center">
        <Parallax speed={-12} className="w-full">
          <div className="animate-swim w-full relative h-[400px]" style={{ animationDuration: '30s' }}>
            ...
            <div className="absolute top-1/2 -translate-y-1/2 left-[20%]">
              <img src={sharkImg} className="w-[20rem] md:w-[30rem] opacity-40" alt="Shark" />
            </div>
          </div>
        </Parallax>
      </div>
        ...
        <div className="max-w-6xl">
          ...
          <header className="mb-24 relative">
            ...
            <div className="flex flex-col items-start relative z-10">
              ...
              <h1 className="text-6xl sm:text-8xl font-black tracking-tighter leading-none text-white drop-shadow-2xl flex flex-row flex-wrap items-center gap-4 md:gap-6">
                <span>WORK</span>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400/80 to-blue-900/30">HISTORY</span>
              </h1>
            </div>
