...
import { type CareerItem, EXPERIENCE } from '../../data';
import FadeInImage from '../ui/FadeInImage';
import Reveal from '../ui/Reveal';

const ExperienceItem = ({ data }: { data: CareerItem }) => {
  return (
    <div className="relative pl-10 md:pl-20 pb-24 last:pb-0 group overflow-hidden">
      ...
      <Reveal className="flex flex-col gap-2 transition-transform duration-500 group-hover:translate-x-1 md:group-hover:translate-x-2 pr-4">
      ...
      <div className="w-full mx-auto px-4 sm:px-8 lg:px-16 xl:px-24 relative z-10">
        <div className="max-w-6xl">
          <header className="mb-20 relative">
            ...
            <Reveal className="flex flex-col items-start relative z-10">
          ...
          <div className="relative w-full">
            {EXPERIENCE.map((exp) => (
              <ExperienceItem key={`${exp.company}-${exp.period}`} data={exp} />
            ))}
          </div>
        </div>
      </div>
